    ...
    myDiagram.contextMenu =
        $("ContextMenu",
            makeButton("Nueva clase", 
                (e, obj) => addNewClass(
                    e.diagram.toolManager.contextMenuTool.mouseDownPoint.x+10, 
                    e.diagram.toolManager.contextMenuTool.mouseDownPoint.y+10
                )
            ),
...
function diagram_save(course_id, nactivity_id, user_email) {    
    ...
    $('#modalTitle').text('Guardando Diagrama...');
    $('#modalResult').text("");
    $('#modalHeader').css("background-color", "white");
    $('#modalBody').css("background-color", "white");
    $('#modalFooter').css("background-color", "white");
...
function diagramVer(course_id, nactivity_id, user_email) 
{    
    alert('VER: '+course_id+' '+nactivity_id+' '+user_email);
    saveDiagramProperties(); // do this first, before writing to JSON
    let dataToSend = {
        course_id: course_id,
        nactivity_id: nactivity_id,
    ...
    $.ajax({
        ...
        success: function(dataJson){
            //Convierte el string JSON en un objeto JAVA.
            data = JSON.parse(dataJson).data;
            alert("El resultado de la evaluación:\n"+JSON.stringify(data));
            if (data.length > 0){
                list = '<b>Problemas encontrados:</b><br>';
                data.forEach(element => {
                    list += element + "<br>";
