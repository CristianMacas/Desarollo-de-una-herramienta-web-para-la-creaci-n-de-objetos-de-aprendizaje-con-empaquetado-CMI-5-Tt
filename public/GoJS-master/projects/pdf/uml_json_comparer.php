...
function check_uml_properties($result, $className, $templateUmlPropertiesList, $testUmlPropertiesList){
    ...
    foreach($templateUmlPropertiesList as $template){
        $found = array_filter($testUmlPropertiesList, function($item) use ($template){
            ...
            return(strtolower($item->name) == strtolower($template->name));
        });
        if (sizeof($found) > 1){
...
function check_uml_links($result, $templateUmlLinksList, $testUmlLinksList, $templateUmlClassesList, $testUmlClassesList)
{
    ...
    foreach($templateUmlLinksList as $template)
    {
        ...
        {
            $encontrado = false;
            foreach($testUmlLinksList as $item)
            {
                if ((property_exists($item, "from")) ||
                    (property_exists($item, "to")) ||
                    (property_exists($item, "relationType"))) 
                {
                    if ( ( strtolower($testUmlClassesList[-1-$item->from]->name) == strtolower($templateUmlClassesList[-1-$template->from]->name) ) &&
                        ( strtolower($testUmlClassesList[-1-$item->to]->name) == strtolower($templateUmlClassesList[-1-$template->to]->name) ) &&
                        ($item->relationType == $template->relationType) )
                    {
                        $encontrado = true;
                        break;
                    }
                    elseif ( ( strtolower($testUmlClassesList[-1-$item->from]->name) == strtolower($templateUmlClassesList[-1-$template->from]->name) ) &&
                        ( strtolower($testUmlClassesList[-1-$item->to]->name) == strtolower($templateUmlClassesList[-1-$template->to]->name) ) &&
                        ($item->relationType != $template->relationType) )
                    {
                        $encontrado = true;
                    ...
                    elseif ( ( strtolower($testUmlClassesList[-1-$item->from]->name) == strtolower($templateUmlClassesList[-1-$template->to]->name) ) &&
                        ( strtolower($testUmlClassesList[-1-$item->to]->name) == strtolower($templateUmlClassesList[-1-$template->from]->name) ) &&
                        ($item->relationType == $template->relationType) )
                    {
                        $encontrado = true;
                        $result[] = 'El enlace entre las clases "'.$templateUmlClassesList[-1-$template->from]->name.'" y "'.$templateUmlClassesList[-1-$template->to]->name.'" esta invertido.';
                    }
                    elseif ( ( strtolower($testUmlClassesList[-1-$item->from]->name) == strtolower($templateUmlClassesList[-1-$template->to]->name) ) &&
                        ( strtolower($testUmlClassesList[-1-$item->to]->name) == strtolower($templateUmlClassesList[-1-$template->from]->name) ) &&
                        ($item->relationType != $template->relationType) )
                    {
                        $encontrado = true;
                        $result[] = 'El enlace entre las clases "'.$templateUmlClassesList[-1-$template->from]->name.'" y "'.$templateUmlClassesList[-1-$template->to]->name.'" esta invertido y debe ser "'.$template->relationType.'".';
                    }
                }
            };
            if (!$encontrado)
    ...
    foreach($testUmlLinksList as $item)
    {
            ...
            (property_exists($item, "relationType")))
        {
            ...
            if (!$encontrado)
            {
                $result[] = 'Sobra el enlace entre las clases "'.$testUmlClassesList[-1-$item->from]->name.'" y "'.$testUmlClassesList[-1-$item->to]->name.'"';
            }
        }
    }
    return($result);
...
function check_uml_diagram($template, $test){
    ...
    try{
        ...
        $result = check_uml_links($result, $template->linkDataArray, $test->linkDataArray, $template->nodeDataArray, $test->nodeDataArray);
    }
    catch(Exception $e){
